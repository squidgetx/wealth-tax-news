...
outlet_stopwords <-
    c(
        "new_york_times", "wall_street_journal", "ny_times", "nyt", "wall_st_journal", "journal", "times", "wsj", "wsj+", "la_times", "washington_post"
    )
...
preprocess <- function(corp) {
    ...
    colocs <- textstat_collocations(tokens, min_count = 2)
...
train_svm <- function(corpus, yvar, trainProp) {
    ...
    train_y <- docvars(corpus_train)[,yvar] %>% as.factor()
    ...
    test_y <- docvars(corpus_test)[,yvar] %>% as.factor()
